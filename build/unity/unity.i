...
static const char UnityStrDetail2Name[] = " " "Argument" " ";
...
void UnityPrintNumberUnsigned(const UNITY_UINT number)
{
    ...
    do
    {
        unity_putc((char)('0' + (number / divisor % 10)));
        divisor /= 10;
    } while (divisor > 0);
}
...
void UnityPrintNumberHex(const UNITY_UINT number, const char nibbles_to_print)
{
    ...
    while (nibbles > 0)
    {
        ...
        if (nibble <= 9)
        {
            unity_putc((char)('0' + nibble));
        }
        else
        {
            unity_putc((char)('A' - 10 + nibble));
        }
    }
}
...
void UnityPrintMask(const UNITY_UINT mask, const UNITY_UINT number)
{
    UNITY_UINT current_bit = (UNITY_UINT)1 << ((32) - 1);
    UNITY_INT32 i;
...
void UnityPrintFloat(const UNITY_DOUBLE input_number)
{
    ...
    else
    {
        ...
        while (digits > 0)
        {
            if(digits == decimals) unity_putc('.');
            unity_putc(buf[--digits]);
        }
...
static void UnityTestResultsBegin(const char* file, const UNITY_UINT line)
{
    UnityPrint(file);
    unity_putc(':');
...
static void UnityTestResultsFailBegin(const UNITY_UINT line)
{
...
static int UnityIsOneArrayNull( const void* expected,
                               const void* actual,
                               const UNITY_UINT lineNumber,
                               const char* msg)
{
                              ...
                              const UNITY_FLAGS_T flags)
{
    ...
    if (num_elements == 0)
    {
        { UnityTestResultsFailBegin(lineNumber); UnityPrint(UnityStrPointless); UnityAddMsgIfSpecified(msg); { Unity.CurrentTestFailed = 1; unity_flush(); longjmp(Unity.AbortFrame, 1); }; };
    }
    ...
    while ((elements > 0) && elements--)
    {
        ...
        {
            case 1:
                expect_val = *( const UNITY_INT8*)expected;
                actual_val = *( const UNITY_INT8*)actual;
                break;
            case 2:
                expect_val = *( const UNITY_INT16*)expected;
                actual_val = *( const UNITY_INT16*)actual;
                break;
            ...
            default:
                expect_val = *( const UNITY_INT32*)expected;
                actual_val = *( const UNITY_INT32*)actual;
                length = 4;
                break;
        }
        ...
        {
            if (style & (0x20) && length < sizeof(expect_val))
            {
                UNITY_INT mask = 1;
        ...
        {
            expected = ( const void*)(length + (const char*)expected);
        }
                                ...
                                const UNITY_FLAGS_T flags)
{
    ...
    if (expected == actual) return;
    if (UnityIsOneArrayNull(( const void*)expected, ( const void*)actual, lineNumber, msg))
        { Unity.CurrentTestFailed = 1; unity_flush(); longjmp(Unity.AbortFrame, 1); };

    while (elements--)
    {
        if (!UnityFloatsWithin(*ptr_expected * (0.00001f), *ptr_expected, *ptr_actual))
        {
            UnityTestResultsFailBegin(lineNumber);
            UnityPrint(UnityStrElement);
            UnityPrintNumberUnsigned(num_elements - elements - 1);
            { UnityPrint(UnityStrExpected); UnityPrintFloat((UNITY_DOUBLE)*ptr_expected); UnityPrint(UnityStrWas); UnityPrintFloat((UNITY_DOUBLE)*ptr_actual); };
            UnityAddMsgIfSpecified(msg);
            { Unity.CurrentTestFailed = 1; unity_flush(); longjmp(Unity.AbortFrame, 1); };
        }
        if (flags == UNITY_ARRAY_TO_ARRAY)
                             ...
                             const UNITY_UINT lineNumber)
{
    ...
    {
        UnityTestResultsFailBegin(lineNumber);
        { UnityPrint(UnityStrExpected); UnityPrintFloat((UNITY_DOUBLE)expected); UnityPrint(UnityStrWas); UnityPrintFloat((UNITY_DOUBLE)actual); };
        UnityAddMsgIfSpecified(msg);
        { Unity.CurrentTestFailed = 1; unity_flush(); longjmp(Unity.AbortFrame, 1); };
    }
}
                                 ...
                                 const UNITY_FLAGS_T flags)
{
    ...
    while (elements--)
    {
        if (!UnityDoublesWithin(*ptr_expected * (1e-12), *ptr_expected, *ptr_actual))
        {
            UnityTestResultsFailBegin(lineNumber);
            UnityPrint(UnityStrElement);
            UnityPrintNumberUnsigned(num_elements - elements - 1);
            { UnityPrint(UnityStrExpected); UnityPrintFloat(*ptr_expected); UnityPrint(UnityStrWas); UnityPrintFloat(*ptr_actual); };
            UnityAddMsgIfSpecified(msg);
            { Unity.CurrentTestFailed = 1; unity_flush(); longjmp(Unity.AbortFrame, 1); };
        }
        if (flags == UNITY_ARRAY_TO_ARRAY)
                              ...
                              const UNITY_UINT lineNumber)
{
    ...
    {
        UnityTestResultsFailBegin(lineNumber);
        { UnityPrint(UnityStrExpected); UnityPrintFloat(expected); UnityPrint(UnityStrWas); UnityPrintFloat(actual); };
        UnityAddMsgIfSpecified(msg);
        { Unity.CurrentTestFailed = 1; unity_flush(); longjmp(Unity.AbortFrame, 1); };
    }
}
                            ...
                            const UNITY_FLAGS_T flags)
{
    ...
    while (elements--)
    {
        ...
        {
            ptr_exp = ( const unsigned char*)expected;
        }
    }
}
 ...
 const void* UnityNumToPtr(const UNITY_INT num, const UNITY_UINT8 size)
{
    switch(size)
    {
        case 1:
          ...
          return ( const void*)(&UnityQuickCompare.i8);

        case 2:
          UnityQuickCompare.i16 = (UNITY_INT16)num;
          return ( const void*)(&UnityQuickCompare.i16);
        ...
        default:
          ...
          return ( const void*)(&UnityQuickCompare.i32);
    }
}
 ...
 const void* UnityFloatToPtr(const float num)
{
    ...
    return ( const void*)(&UnityQuickCompare.f);
}
 ...
 const void* UnityDoubleToPtr(const double num)
{
    ...
    return ( const void*)(&UnityQuickCompare.d);
}
...
# 1321 "/home/dieter/SoftwareDevelop/oxypoint-am/Prerequisites/esp-idf/components/unity/unity/src/unity.c"
    ...
    { Unity.CurrentTestIgnored = 1; unity_flush(); longjmp(Unity.AbortFrame, 1); };
}
