# 1 "/home/dieter/SoftwareDevelop/oxypoint-am/Prerequisites/esp-idf/components/tcp_transport/transport_ws.c"
...
# 2 "/home/dieter/SoftwareDevelop/oxypoint-am/Prerequisites/esp-idf/components/tcp_transport/transport_ws.c" 2
...
# 3 "/home/dieter/SoftwareDevelop/oxypoint-am/Prerequisites/esp-idf/components/tcp_transport/transport_ws.c" 2
...
# 4 "/home/dieter/SoftwareDevelop/oxypoint-am/Prerequisites/esp-idf/components/tcp_transport/transport_ws.c" 2
# 1 "/home/dieter/SoftwareDevelop/oxypoint-am/Prerequisites/esp-idf/components/newlib/platform_include/sys/random.h" 1
# 18 "/home/dieter/SoftwareDevelop/oxypoint-am/Prerequisites/esp-idf/components/newlib/platform_include/sys/random.h"
...
# 19 "/home/dieter/SoftwareDevelop/oxypoint-am/Prerequisites/esp-idf/components/newlib/platform_include/sys/random.h" 2
...
# 5 "/home/dieter/SoftwareDevelop/oxypoint-am/Prerequisites/esp-idf/components/tcp_transport/transport_ws.c" 2
...
# 8 "/home/dieter/SoftwareDevelop/oxypoint-am/Prerequisites/esp-idf/components/tcp_transport/transport_ws.c" 2
...
static char *trimwhitespace(const char *str)
{
    ...
    while ((((__ctype_ptr__+sizeof(""[(unsigned char)*str]))[(int)((unsigned char)*str)])&010)) str++;
    ...
    while (end > str && (((__ctype_ptr__+sizeof(""[(unsigned char)*end]))[(int)((unsigned char)*end)])&010)) end--;
...
static int ws_connect(esp_transport_handle_t t, const char *host, int port, int timeout_ms)
{
    ...
    int len = snprintf(ws->buffer, (1024),
                         "GET %s HTTP/1.1\r\n"
                         "Connection: Upgrade\r\n"
                         "Host: %s:%d\r\n"
                         "Upgrade: websocket\r\n"
...
# 171 "/home/dieter/SoftwareDevelop/oxypoint-am/Prerequisites/esp-idf/components/tcp_transport/transport_ws.c"
    ...
    do { if ( 3 >= ESP_LOG_DEBUG ) do { if (ESP_LOG_DEBUG==ESP_LOG_ERROR ) { esp_log_write(ESP_LOG_ERROR, TAG, "\033[0;" "31" "m" "E" " (%d) %s: " "server key=%s, send_key=%s, expected_server_key=%s" "\033[0m" "\n", esp_log_timestamp(), TAG, (char *)server_key, (char*)client_key, expected_server_key); } else if (ESP_LOG_DEBUG==ESP_LOG_WARN ) { esp_log_write(ESP_LOG_WARN, TAG, "\033[0;" "33" "m" "W" " (%d) %s: " "server key=%s, send_key=%s, expected_server_key=%s" "\033[0m" "\n", esp_log_timestamp(), TAG, (char *)server_key, (char*)client_key, expected_server_key); } else if (ESP_LOG_DEBUG==ESP_LOG_DEBUG ) { esp_log_write(ESP_LOG_DEBUG, TAG, "D" " (%d) %s: " "server key=%s, send_key=%s, expected_server_key=%s" "\033[0m" "\n", esp_log_timestamp(), TAG, (char *)server_key, (char*)client_key, expected_server_key); } else if (ESP_LOG_DEBUG==ESP_LOG_VERBOSE ) { esp_log_write(ESP_LOG_VERBOSE, TAG, "V" " (%d) %s: " "server key=%s, send_key=%s, expected_server_key=%s" "\033[0m" "\n", esp_log_timestamp(), TAG, (char *)server_key, (char*)client_key, expected_server_key); } else { esp_log_write(ESP_LOG_INFO, TAG, "\033[0;" "32" "m" "I" " (%d) %s: " "server key=%s, send_key=%s, expected_server_key=%s" "\033[0m" "\n", esp_log_timestamp(), TAG, (char *)server_key, (char*)client_key, expected_server_key); } } while(0); } while(0);
    if (strcmp((char*)expected_server_key, (char*)server_key) != 0) {
        do { if ( 3 >= ESP_LOG_ERROR ) do { if (ESP_LOG_ERROR==ESP_LOG_ERROR ) { esp_log_write(ESP_LOG_ERROR, TAG, "\033[0;" "31" "m" "E" " (%d) %s: " "Invalid websocket key" "\033[0m" "\n", esp_log_timestamp(), TAG); } else if (ESP_LOG_ERROR==ESP_LOG_WARN ) { esp_log_write(ESP_LOG_WARN, TAG, "\033[0;" "33" "m" "W" " (%d) %s: " "Invalid websocket key" "\033[0m" "\n", esp_log_timestamp(), TAG); } else if (ESP_LOG_ERROR==ESP_LOG_DEBUG ) { esp_log_write(ESP_LOG_DEBUG, TAG, "D" " (%d) %s: " "Invalid websocket key" "\033[0m" "\n", esp_log_timestamp(), TAG); } else if (ESP_LOG_ERROR==ESP_LOG_VERBOSE ) { esp_log_write(ESP_LOG_VERBOSE, TAG, "V" " (%d) %s: " "Invalid websocket key" "\033[0m" "\n", esp_log_timestamp(), TAG); } else { esp_log_write(ESP_LOG_INFO, TAG, "\033[0;" "32" "m" "I" " (%d) %s: " "Invalid websocket key" "\033[0m" "\n", esp_log_timestamp(), TAG); } } while(0); } while(0);
        return -1;
    }
    return 0;
}
