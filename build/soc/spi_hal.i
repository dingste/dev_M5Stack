# 1 "/home/dieter/Development/esp-idf/components/soc/src/hal/spi_hal.c"
# 1 "/home/dieter/Development/ProjektEi/build/soc//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/home/dieter/Development/esp-idf/components/soc/src/hal/spi_hal.c"
# 17 "/home/dieter/Development/esp-idf/components/soc/src/hal/spi_hal.c"
...
# 18 "/home/dieter/Development/esp-idf/components/soc/src/hal/spi_hal.c" 2
...
esp_err_t spi_hal_get_clock_conf(const spi_hal_context_t *hal, int speed_hz, int duty_cycle, 
# 52 "/home/dieter/Development/esp-idf/components/soc/src/hal/spi_hal.c" 3 4
                                                                                            _Bool 
# 52 "/home/dieter/Development/esp-idf/components/soc/src/hal/spi_hal.c"
                                                                                                 use_gpio, int input_delay_ns, int *out_freq, spi_hal_timing_conf_t *timing_conf)
{
    spi_hal_timing_conf_t temp_conf;

    int eff_clk_n = spi_ll_master_cal_clock(( 80*1000000 ), speed_hz, duty_cycle, &temp_conf.clock_reg);
    ...
    if (!(hal->half_duplex || temp_conf.timing_dummy == 0 || hal->no_compensate)) { do { if ( 3 >= ESP_LOG_ERROR ) do { if (ESP_LOG_ERROR==ESP_LOG_ERROR ) { esp_log_write(ESP_LOG_ERROR, SPI_HAL_TAG, "\033[0;" "31" "m" "E" " (%d) %s: " "%s(%d): ""When work in full-duplex mode at frequency > %.1fMHz, device cannot read correct data.\nTry to use IOMUX pins to increase the frequency limit, or use the half duplex mode.\nPlease note the SPI master can only work at divisors of 80MHz, and the driver always tries to find the closest frequency to your configuration.\nSpecify ``SPI_DEVICE_NO_DUMMY`` to ignore this checking. Then you can output data at higher speed, or read data at your own risk." "\033[0m" "\n", esp_log_timestamp(), SPI_HAL_TAG, __FUNCTION__, 70, freq_limit / 1000. / 1000); } else if (ESP_LOG_ERROR==ESP_LOG_WARN ) { esp_log_write(ESP_LOG_WARN, SPI_HAL_TAG, "\033[0;" "33" "m" "W" " (%d) %s: " "%s(%d): ""When work in full-duplex mode at frequency > %.1fMHz, device cannot read correct data.\nTry to use IOMUX pins to increase the frequency limit, or use the half duplex mode.\nPlease note the SPI master can only work at divisors of 80MHz, and the driver always tries to find the closest frequency to your configuration.\nSpecify ``SPI_DEVICE_NO_DUMMY`` to ignore this checking. Then you can output data at higher speed, or read data at your own risk." "\033[0m" "\n", esp_log_timestamp(), SPI_HAL_TAG, __FUNCTION__, 70, freq_limit / 1000. / 1000); } else if (ESP_LOG_ERROR==ESP_LOG_DEBUG ) { esp_log_write(ESP_LOG_DEBUG, SPI_HAL_TAG, "D" " (%d) %s: " "%s(%d): ""When work in full-duplex mode at frequency > %.1fMHz, device cannot read correct data.\nTry to use IOMUX pins to increase the frequency limit, or use the half duplex mode.\nPlease note the SPI master can only work at divisors of 80MHz, and the driver always tries to find the closest frequency to your configuration.\nSpecify ``SPI_DEVICE_NO_DUMMY`` to ignore this checking. Then you can output data at higher speed, or read data at your own risk." "\033[0m" "\n", esp_log_timestamp(), SPI_HAL_TAG, __FUNCTION__, 70, freq_limit / 1000. / 1000); } else if (ESP_LOG_ERROR==ESP_LOG_VERBOSE ) { esp_log_write(ESP_LOG_VERBOSE, SPI_HAL_TAG, "V" " (%d) %s: " "%s(%d): ""When work in full-duplex mode at frequency > %.1fMHz, device cannot read correct data.\nTry to use IOMUX pins to increase the frequency limit, or use the half duplex mode.\nPlease note the SPI master can only work at divisors of 80MHz, and the driver always tries to find the closest frequency to your configuration.\nSpecify ``SPI_DEVICE_NO_DUMMY`` to ignore this checking. Then you can output data at higher speed, or read data at your own risk." "\033[0m" "\n", esp_log_timestamp(), SPI_HAL_TAG, __FUNCTION__, 70, freq_limit / 1000. / 1000); } else { esp_log_write(ESP_LOG_INFO, SPI_HAL_TAG, "\033[0;" "32" "m" "I" " (%d) %s: " "%s(%d): ""When work in full-duplex mode at frequency > %.1fMHz, device cannot read correct data.\nTry to use IOMUX pins to increase the frequency limit, or use the half duplex mode.\nPlease note the SPI master can only work at divisors of 80MHz, and the driver always tries to find the closest frequency to your configuration.\nSpecify ``SPI_DEVICE_NO_DUMMY`` to ignore this checking. Then you can output data at higher speed, or read data at your own risk." "\033[0m" "\n", esp_log_timestamp(), SPI_HAL_TAG, __FUNCTION__, 70, freq_limit / 1000. / 1000); } } while(0); } while(0); return (0x106); }
...
int spi_hal_master_cal_clock(int fapb, int hz, int duty_cycle)
{
    return spi_ll_master_cal_clock(fapb, hz, duty_cycle, 
# 84 "/home/dieter/Development/esp-idf/components/soc/src/hal/spi_hal.c" 3 4
                                                        ((void *)0)
# 84 "/home/dieter/Development/esp-idf/components/soc/src/hal/spi_hal.c"
                                                            );
}

void spi_hal_cal_timing(int eff_clk, 
# 87 "/home/dieter/Development/esp-idf/components/soc/src/hal/spi_hal.c" 3 4
                                    _Bool 
# 87 "/home/dieter/Development/esp-idf/components/soc/src/hal/spi_hal.c"
                                         gpio_is_used, int input_delay_ns, int *dummy_n, int *miso_delay_n)
{
    ...
    *miso_delay_n = miso_delay;
    do { if ( 3 >= ESP_LOG_DEBUG ) do { if (ESP_LOG_DEBUG==ESP_LOG_ERROR ) { esp_log_write(ESP_LOG_ERROR, SPI_HAL_TAG, "\033[0;" "31" "m" "E" " (%d) %s: " "eff: %d, limit: %dk(/%d), %d dummy, %d delay" "\033[0m" "\n", esp_log_timestamp(), SPI_HAL_TAG, eff_clk / 1000, apbclk_kHz / (apb_period_n + 1), apb_period_n, dummy_required, miso_delay); } else if (ESP_LOG_DEBUG==ESP_LOG_WARN ) { esp_log_write(ESP_LOG_WARN, SPI_HAL_TAG, "\033[0;" "33" "m" "W" " (%d) %s: " "eff: %d, limit: %dk(/%d), %d dummy, %d delay" "\033[0m" "\n", esp_log_timestamp(), SPI_HAL_TAG, eff_clk / 1000, apbclk_kHz / (apb_period_n + 1), apb_period_n, dummy_required, miso_delay); } else if (ESP_LOG_DEBUG==ESP_LOG_DEBUG ) { esp_log_write(ESP_LOG_DEBUG, SPI_HAL_TAG, "D" " (%d) %s: " "eff: %d, limit: %dk(/%d), %d dummy, %d delay" "\033[0m" "\n", esp_log_timestamp(), SPI_HAL_TAG, eff_clk / 1000, apbclk_kHz / (apb_period_n + 1), apb_period_n, dummy_required, miso_delay); } else if (ESP_LOG_DEBUG==ESP_LOG_VERBOSE ) { esp_log_write(ESP_LOG_VERBOSE, SPI_HAL_TAG, "V" " (%d) %s: " "eff: %d, limit: %dk(/%d), %d dummy, %d delay" "\033[0m" "\n", esp_log_timestamp(), SPI_HAL_TAG, eff_clk / 1000, apbclk_kHz / (apb_period_n + 1), apb_period_n, dummy_required, miso_delay); } else { esp_log_write(ESP_LOG_INFO, SPI_HAL_TAG, "\033[0;" "32" "m" "I" " (%d) %s: " "eff: %d, limit: %dk(/%d), %d dummy, %d delay" "\033[0m" "\n", esp_log_timestamp(), SPI_HAL_TAG, eff_clk / 1000, apbclk_kHz / (apb_period_n + 1), apb_period_n, dummy_required, miso_delay); } } while(0); } while(0);
}

int spi_hal_get_freq_limit(
# 118 "/home/dieter/Development/esp-idf/components/soc/src/hal/spi_hal.c" 3 4
                          _Bool 
# 118 "/home/dieter/Development/esp-idf/components/soc/src/hal/spi_hal.c"
                               gpio_is_used, int input_delay_ns)
{
    const int apbclk_kHz = ( 80*1000000 ) / 1000;
    const int gpio_delay_ns = gpio_is_used ? 25 : 0;
